...
#define ASSERT_EQUALS_RECORDS(_l,_r, schema, message)			\
  do {									\
    Record *_lR = _l;                                                   \
    Record *_rR = _r;                                                   \
    ASSERT_TRUE(memcmp(_lR->data,_rR->data,1) == 0, message); \
     int i;								\
...
testRecords (void)
{
  ...
  value1=stringToValue("i1");
  getAttr(r, schema, 0, &value);
  OP_TRUE(value1, value, valueEquals, "first attr");
  freeVal(value);
  freeVal(value1);

  value1=stringToValue("saaaa");
  getAttr(r, schema, 1, &value);
  OP_TRUE(value1, value, valueEquals, "second attr");
  freeVal(value);
  freeVal(value1);

  value1=stringToValue("i3");
  getAttr(r, schema, 2, &value);
  OP_TRUE(value1, value, valueEquals, "third attr");
  freeVal(value);
  freeVal(value1);

  //modify attrs
  value1=stringToValue("i4");
 setAttr(r, schema, 2,value1 );
 freeVal(value1);
  getAttr(r, schema, 2, &value);

  value1=stringToValue("i4");
  OP_TRUE(value1, value, valueEquals, "third attr after setting");
  freeVal(value);
  freeVal(value1);
  freeSchema(schema);
  freeRecord(r);
...
testUpdateTable (void)
{
  ...
  for(i = 0; i < numInserts; i++)
    {
      ...
      TEST_CHECK(insertRecord(table,r));
      rids[i] = r->id;
      printf("Insert RID page %d \n",rids[i].page);
      printf("Insert RID slot %d \n",rids[i].slot);
      free(r);
    }
 ...
 for(i = 0; i < numUpdates; i++)
  ...
  for(i = 0; i < numFinal; i++)
    {
	  r2=fromTestRecord(schema, finalR[i]);
      RID rid = rids[i];
      createRecord(&r1,schema);
      TEST_CHECK(getRecord(table, rid, r1));
      ASSERT_EQUALS_RECORDS(r2, r1, schema, "compare records");
      //free(r1);
    }
 ...
 TEST_CHECK(closeTable(table));
  ...
  // retrieve records from the table and compare to expected final stage
    ...
    for(i = 0; i < numInserts; i++)
    {
      ...
      createRecord(&r1,schema);
      TEST_CHECK(getRecord(table, rid,r1));
      r4=fromTestRecord(schema, realInserts[i]);
      ASSERT_EQUALS_RECORDS(r4, r1, schema, "compare records");
      free(r1);
      freeRecord(r4);
    }

  r = fromTestRecord(schema, updates[0]);
  r3=fromTestRecord(schema, updates[0]);
  r->id = rids[randomRec];
  TEST_CHECK(updateRecord(table,r));
  free(r);
  createRecord(&r2,schema);
  TEST_CHECK(getRecord(table, rids[randomRec], r2));

  ASSERT_EQUALS_RECORDS(r3, r2, schema, "compare records");
  TEST_CHECK(closeTable(table));
